      ...
      <table className="w-full text-sm text-left">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-100">
            ...
            <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-center">Icon</th>
            <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-center">Title</th>
            <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-center">Description</th>
        ...
        <tbody className="divide-y divide-gray-50">
          {(data || []).map((feature, index) => {
            ...
            const title = feature?.translations?.[0]?.title || "—";
            const desc = feature?.translations?.[0]?.description || "—";
                ...
                <td className="px-5 py-3 text-center">
                  <div className="flex justify-center">
                    <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center overflow-hidden">
                      {iconUrl
                        ? <img src={iconUrl} className="w-7 h-7 object-contain" alt={title} />
                        : <Icon icon="mdi:airplane-takeoff" className="text-indigo-400" width={20} />}
                    </div>
                  </div>
                </td>
                <td className="px-5 py-4 text-center text-sm font-medium text-gray-700">{title}</td>
                <td className="px-5 py-4 text-center text-xs text-gray-500 max-w-xs">
                  <span className="line-clamp-2">{desc}</span>
                </td>
                <td className="px-5 py-4 text-center">
                  <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full ${feature.feature_active ? "bg-emerald-100 text-emerald-600" : "bg-gray-100 text-gray-500"}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${feature.feature_active ? "bg-emerald-500" : "bg-gray-400"}`} />
                  ...
                  <div className="relative inline-block" ref={(el) => (dropdownRefs.current[index] = el)}>
                    ...
                    {openIndex === index && (
                      <div className={`absolute right-0 z-50 bg-white rounded-xl shadow-lg border border-gray-100 w-40 overflow-hidden ${isLast ? "bottom-full mb-2" : "mt-2"}`}>
                        <button onClick={() => { setOpenIndex(null); onEdit?.(feature); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-blueMain/5 hover:text-blueMain transition">
