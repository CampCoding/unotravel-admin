...
export default function FareFlightFeatureForm({ initialData, onSuccess, onClose }) {
  const isEdit = !!initialData;
  const id = initialData?.feature_id || initialData?.id;

  const [iconFile, setIconFile] = useState(null);
  const [active, setActive] = useState(initialData?.feature_active ?? true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (iconFile) fd.append("feature_icon", iconFile);
      fd.append("feature_active", active ? "1" : "0");
      fd.append("sort_order", String(sortOrder));
      fd.append("translations", JSON.stringify(translations));
      if (isEdit) await fareFlightFeaturesAPI.update(id, fd);
      else await fareFlightFeaturesAPI.create(fd);
      onSuccess();
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <FileUpload label="Feature Icon" value={initialData?.feature_icon} onChange={setIconFile} />
      <div>
