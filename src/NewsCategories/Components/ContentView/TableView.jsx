      ...
      <table className="w-full text-sm text-left">
        ...
        <tbody className="divide-y divide-gray-50">
          {(data || []).map((cat, index) => {
            ...
            const name = cat?.translations?.[0]?.category_name || "—";
            return (
              <tr key={id} className="hover:bg-blue-50/30 transition-colors">
                ...
                <td className="px-5 py-4 text-center">
                  <span className="font-mono text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-lg">{cat.category_key || "—"}</span>
                </td>
                <td className="px-5 py-4 text-center text-xs text-gray-400">#{cat.sort_order ?? "—"}</td>
                <td className="px-5 py-4 text-center">
                  <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full ${cat.category_active ? "bg-emerald-100 text-emerald-600" : "bg-gray-100 text-gray-500"}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${cat.category_active ? "bg-emerald-500" : "bg-gray-400"}`} />
                    {cat.category_active ? "Active" : "Inactive"}
                  </span>
                </td>
                <td className="px-5 py-4 text-center">
                  <div className="relative inline-block" ref={(el) => (dropdownRefs.current[index] = el)}>
                    ...
                    {openIndex === index && (
                      <div className={`absolute right-0 z-50 bg-white rounded-xl shadow-lg border border-gray-100 w-40 overflow-hidden ${isLast ? "bottom-full mb-2" : "mt-2"}`}>
                        <button onClick={() => { setOpenIndex(null); onEdit?.(cat); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-blueMain/5 hover:text-blueMain transition">
