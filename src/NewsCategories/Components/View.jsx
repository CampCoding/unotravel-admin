...
export default function View() {
  ...
  const { data, loading, error, refetch } = useFetch(newsCategoriesAPI.list);
  ...
  const handleDelete = async (id) => {
    ...
    try { await newsCategoriesAPI.delete(id); refetch(); } catch { alert("Failed to delete category."); }
  };
  ...
  return (
    <section className="flex-1 p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-base font-semibold text-gray-700">News Categories</h2>
          <p className="text-xs text-gray-400 mt-0.5">{loading ? "Loading..." : `${data?.length ?? 0} categories total`}</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleAdd} className="inline-flex items-center gap-2 px-4 py-2.5 bg-blueMain text-white text-sm font-semibold rounded-xl shadow-md hover:bg-blue-700 active:scale-95 transition-all"><Icon icon="mdi:plus" width={16} /> Add Category</button>
        </div>
      </div>
      {loading && <LoadingGrid count={6} />}
      {error && <ErrorState message={error} onRetry={refetch} />}
      {!loading && !error && (!data || data.length === 0) && <EmptyState icon="mdi:tag-text-outline" message="No categories yet" action={handleAdd} actionLabel="Add Category" />}
      {!loading && !error && data && data.length > 0 && <Content view={view} data={data} onDelete={handleDelete} onEdit={handleEdit} />}
      <Modal open={openModal} onClose={handleClose} title={editItem ? "Edit Category" : "Add Category"} icon="mdi:tag-text-outline">
        <CategoryForm initialData={editItem} onSuccess={handleSuccess} onClose={handleClose} />
      </Modal>
    </section>
  );
}
