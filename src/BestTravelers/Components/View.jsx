...
import BestTravelerForm from "./Form.jsx";
...
export default function View() {
  ...
  const { data, loading, error, refetch } = useFetch(bestTravelersAPI.list);
  ...
  const handleDelete = async (id) => {
    ...
    try { await bestTravelersAPI.delete(id); refetch(); } catch { alert("Failed to delete traveler."); }
  };
  ...
  return (
    <section className="flex-1 p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-base font-semibold text-gray-700">Best Travelers</h2>
          <p className="text-xs text-gray-400 mt-0.5">{loading ? "Loading..." : `${data?.length ?? 0} travelers total`}</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleAdd} className="inline-flex items-center gap-2 px-4 py-2.5 bg-blueMain text-white text-sm font-semibold rounded-xl shadow-md hover:bg-blue-700 active:scale-95 transition-all"><Icon icon="mdi:plus" width={16} /> Add Traveler</button>
        </div>
      </div>
      {loading && <LoadingGrid count={6} />}
      {error && <ErrorState message={error} onRetry={refetch} />}
      {!loading && !error && (!data || data.length === 0) && <EmptyState icon="mdi:account-star-outline" message="No travelers yet" action={handleAdd} actionLabel="Add Traveler" />}
      {!loading && !error && data && data.length > 0 && <Content view={view} data={data} onDelete={handleDelete} onEdit={handleEdit} />}
      <Modal open={openModal} onClose={handleClose} title={editItem ? "Edit Traveler" : "Add Traveler"} icon="mdi:account-star-outline">
        <BestTravelerForm initialData={editItem} onSuccess={handleSuccess} onClose={handleClose} />
      </Modal>
    </section>
  );
}
