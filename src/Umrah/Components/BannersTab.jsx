...
export default function BannersTab() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      fd.append("umrah_image", file);
      await umrahAPI.bannerCreate(fd);
      refetch();
  ...
  const handleDelete = async (id) => {
    ...
    await umrahAPI.bannerDelete(id);
    refetch();
  };
  ...
  const toggleActive = async (banner) => {
    ...
    await umrahAPI.bannerUpdate(banner.id, fd);
    refetch();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {banners.length === 0 && (
        <div className="text-center py-16 text-gray-400">No banners yet. Upload one above.</div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {banners.map((b) => (
          <div key={b.id} className={`relative rounded-2xl overflow-hidden border-2 transition ${b.is_active ? "border-blueMain" : "border-gray-200 opacity-60"}`}>
            <img src={b.image_url} alt="" className="w-full h-48 object-cover" />
