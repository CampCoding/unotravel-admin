...
const EMPTY_FORM = { title: "", subtitle: "", sort_order: "", is_active: true };

export default function BannersTab() {
  const { data: raw, loading, error, refetch } = useFetch(intlToursAPI.bannersList);
  ...
  const openEdit = (b) => {
    setEditing(b);
    setForm({ title: b.title ?? "", subtitle: b.subtitle ?? "", sort_order: b.sort_order ?? "", is_active: !!b.is_active });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (file) fd.append("intl_tours_image", file);
      fd.append("title",      form.title);
      fd.append("subtitle",   form.subtitle);
      fd.append("sort_order", form.sort_order || 0);
      fd.append("is_active",  form.is_active ? 1 : 0);
      if (editing) await intlToursAPI.bannerUpdate(editing.id, fd);
      else         await intlToursAPI.bannerCreate(fd);
      refetch(); closeModal();
  ...
  const handleDelete = async (id) => {
    ...
    await intlToursAPI.bannerDelete(id); refetch();
  };

  const toggleActive = async (b) => {
    const fd = new FormData();
    fd.append("is_active", b.is_active ? 0 : 1);
    await intlToursAPI.bannerUpdate(b.id, fd); refetch();
  };
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={form.title} onChange={(e) => setForm(p => ({ ...p, title: e.target.value }))} placeholder="e.g. Explore the World"
              ...
              <div>
                ...
                <input type="text" value={form.subtitle} onChange={(e) => setForm(p => ({ ...p, subtitle: e.target.value }))} placeholder="e.g. Premium international tour packages"
                  className="w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blueMain/40" />
              </div>
