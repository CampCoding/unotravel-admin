...
export default function MinorsLoungeForm({ initialData, onSuccess, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const body = { item_active: active ? 1 : 0, sort_order: Number(sortOrder), translations };
      if (isEdit) await minorsLoungeAPI.update(id, body);
      else await minorsLoungeAPI.create(body);
      onSuccess();
