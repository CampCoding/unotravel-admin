...
import TravelSupportForm from "./Form.jsx";
...
import { travelSupportAPI } from "../../api/endpoints.js";
...
export default function View() {
  ...
  const { data, loading, error, refetch } = useFetch(travelSupportAPI.list);
  ...
  const handleDelete = async (id) => {
    ...
    try { await travelSupportAPI.delete(id); refetch(); } catch { alert("Failed to delete item."); }
  };
  ...
  return (
    <section className="flex-1 p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-base font-semibold text-gray-700">Travel Support Items</h2>
      ...
      {!loading && !error && (!data || data.length === 0) && <EmptyState icon="mdi:lifebuoy" message="No support items yet" action={handleAdd} actionLabel="Add Item" />}
      {!loading && !error && data && data.length > 0 && <Content view={view} data={data} onDelete={handleDelete} onEdit={handleEdit} />}
      <Modal open={openModal} onClose={handleClose} title={editItem ? "Edit Support Item" : "Add Support Item"} icon="mdi:lifebuoy">
        <TravelSupportForm initialData={editItem} onSuccess={handleSuccess} onClose={handleClose} />
      </Modal>
    </section>
  );
}
