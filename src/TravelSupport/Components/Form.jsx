...
export default function TravelSupportForm({ initialData, onSuccess, onClose }) {
  const isEdit = !!initialData;
  const id = initialData?.id;

  const [itemIcon, setItemIcon] = useState(initialData?.item_icon || "");
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const body = { item_icon: itemIcon, item_active: active ? 1 : 0, sort_order: Number(sortOrder), translations };
      if (isEdit) await travelSupportAPI.update(id, body);
      else await travelSupportAPI.create(body);
      onSuccess();
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1.5">Icon Class (FontAwesome)</label>
        <input type="text" value={itemIcon} onChange={(e) => setItemIcon(e.target.value)} placeholder="fa-headset" className={inp} />
        <p className="text-xs text-gray-400 mt-1">e.g. fa-headset, fa-plane, fa-shield-alt</p>
      </div>
      <div>
