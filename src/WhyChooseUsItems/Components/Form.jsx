...
export default function WhyChooseUsItemForm({ initialData, onSuccess, onClose }) {
  const isEdit = !!initialData;
  const id = initialData?.item_id || initialData?.id;

  const [iconFile, setIconFile] = useState(null);
  const [itemKey, setItemKey] = useState(initialData?.item_key || "");
  const [active, setActive] = useState(initialData?.item_active ?? true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (iconFile) fd.append("item_icon", iconFile);
      fd.append("item_key", itemKey);
      fd.append("item_active", active ? "1" : "0");
      fd.append("show_in_home", showInHome ? "1" : "0");
      fd.append("sort_order", String(sortOrder));
      fd.append("translations", JSON.stringify(translations));
      if (isEdit) await whyChooseUsItemsAPI.update(id, fd);
      else await whyChooseUsItemsAPI.create(fd);
      onSuccess();
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <FileUpload label="Icon Image" value={initialData?.item_icon} onChange={setIconFile} />
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1.5">Item Key</label>
        <input type="text" value={itemKey} onChange={(e) => setItemKey(e.target.value)} placeholder="best-prices" className={inp} />
      </div>
      <div>
