...
import WhyChooseUsItemForm from "./Form.jsx";
...
import { whyChooseUsItemsAPI } from "../../api/endpoints.js";
...
export default function View() {
  ...
  const { data, loading, error, refetch } = useFetch(whyChooseUsItemsAPI.list);
  ...
  const handleDelete = async (id) => {
    ...
    try { await whyChooseUsItemsAPI.delete(id); refetch(); } catch { alert("Failed to delete item."); }
  };
  ...
  return (
    <section className="flex-1 p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-base font-semibold text-gray-700">Why Choose Us Items</h2>
      ...
      {!loading && !error && (!data || data.length === 0) && <EmptyState icon="mdi:star-check-outline" message="No items yet" action={handleAdd} actionLabel="Add Item" />}
      {!loading && !error && data && data.length > 0 && <Content view={view} data={data} onDelete={handleDelete} onEdit={handleEdit} />}
      <Modal open={openModal} onClose={handleClose} title={editItem ? "Edit Item" : "Add Item"} icon="mdi:star-check-outline">
        <WhyChooseUsItemForm initialData={editItem} onSuccess={handleSuccess} onClose={handleClose} />
      </Modal>
    </section>
  );
}
