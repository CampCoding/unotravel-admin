...
const EMPTY = { slug: "", name: "", is_active: 1, sort_order: 0 };

function PassportTypeForm({ item, onClose, onSaved }) {
  const isEdit = !!item;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) await visaAPI.passportTypeUpdate(item.id, form);
      else        await visaAPI.passportTypeCreate(form);
      onSaved(); onClose();
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h3 className="font-bold text-gray-800">{isEdit ? "Edit Passport Type" : "Add Passport Type"}</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3">
          <div>
            ...
            <input value={form.slug} onChange={e => set("slug", e.target.value.toLowerCase().replace(/\s+/g,"-"))} required className={inp} placeholder="ordinary" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1">Name *</label>
            <input value={form.name} onChange={e => set("name", e.target.value)} required className={inp} placeholder="Ordinary Passport" />
          </div>
...
export default function PassportTypesTab() {
  const { data: raw, loading, refetch } = useFetch(visaAPI.passportTypesList);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Delete this passport type?")) return;
    await visaAPI.passportTypeDelete(id); refetch();
  };
  ...
  return (
    <>
      {(showForm || edit) && (
        <PassportTypeForm item={edit} onClose={() => { setEdit(null); setShowForm(false); }} onSaved={refetch} />
      )}
      <div className="flex justify-end mb-4">
        <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-blueMain text-white text-sm font-semibold px-4 py-2 rounded-xl hover:bg-blue-700 transition">
          <Icon icon="mdi:plus" width={15} /> Add Passport Type
        </button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {items.length === 0 && (
          <div className="col-span-3 text-center py-12 text-gray-400 border-2 border-dashed border-gray-200 rounded-2xl">No passport types yet.</div>
        )}
      </div>
    </>
  );
