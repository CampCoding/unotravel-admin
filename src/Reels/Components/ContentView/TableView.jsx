...
export default function TableView({ videos, onRefetch, onEdit }) {
      ...
      <table className="w-full text-sm text-left">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-100">
            ...
            <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider text-center">Preview</th>
              ...
              <tr key={video.id} className="hover:bg-blue-50/30 transition-colors">
                ...
                <td className="px-5 py-3 text-center">
                  <div className="flex justify-center">
                    <div className="w-10 h-16 rounded-xl bg-gray-900 overflow-hidden shadow-sm">
                      {thumbnailUrl ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <Icon icon="bxs:videos" className="text-white/40" width={18} />
                        </div>
                      )}
                    </div>
                  </div>
                </td>
                <td className="px-5 py-4 text-center text-sm font-medium text-gray-700">{video.title || `Reel #${video.id}`}</td>
                <td className="px-5 py-4 text-center">
                  <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full ${video.active ? "bg-emerald-100 text-emerald-600" : "bg-gray-100 text-gray-500"}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${video.active ? "bg-emerald-500" : "bg-gray-400"}`} />
                  ...
                  <div className="relative inline-block text-left" ref={(el) => (dropdownRefs.current[index] = el)}>
                    ...
                    {openIndex === index && (
                      <div className={`absolute right-0 z-50 bg-white rounded-xl shadow-lg border border-gray-100 w-40 overflow-hidden animate-fade-in ${isLast ? "bottom-full mb-2" : "mt-2"}`}>
                        <button onClick={() => { setOpenIndex(null); onEdit?.(video); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-blueMain/5 hover:text-blueMain transition">
                          <Icon icon="flowbite:edit-solid" width={15} className="text-blueMain" />
                          Edit
                        </button>
                        <button onClick={() => { setOpenIndex(null); handleDelete(video.id); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-red-50 hover:text-red-500 transition">
